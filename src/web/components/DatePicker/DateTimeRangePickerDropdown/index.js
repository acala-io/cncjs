...
class DateTimeRangePickerDropdown extends PureComponent {
  ...
  handleDropdownSelect = eventKey => {
    ...
    if (period !== this.state.period) {
      ...
      const startDate = moment(startOfDay)
        .subtract(days > 0 ? days - 1 : 0, 'days')
        .format('YYYY-MM-DD');
      const startTime = moment(startOfDay)
        .subtract(days > 0 ? days - 1 : 0, 'days')
        .format('HH:mm:ss');
  ...
  render() {
    ...
    const {nextStartDate, nextStartTime, nextEndDate, nextEndTime} = this.state;
    ...
    return (
      ...
      >
        <Dropdown.Toggle>{mapPeriodToString(period)}</Dropdown.Toggle>
        <Dropdown.MenuWrapper style={{whiteSpace: 'nowrap'}}>
          <Dropdown.Menu>
            ...
            <MenuItem divider />
            <MenuItem eventKey="custom">{mapPeriodToString('custom')}</MenuItem>
          </Dropdown.Menu>
